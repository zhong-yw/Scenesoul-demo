...
**白界 · Scenesoul** — 一个"虚拟生命"框架（v0.6）。核心是两个 Agent 协作：**大脑（Brain）** 作为意识体，**界说（Narrator）** 作为世界构造者。大脑产生内心独白并与用户对话，界说观察大脑的思绪，为其构建场景并推动世界发展。
...
- `MEMORY_ENABLED` — 是否启用跨会话记忆写入与恢复（默认 1）
...
```
main.py                   — 入口：CLI 交互循环（ScenesoulLoop 包装 Runtime + 终端渲染）
runtime/scenesoul_runtime.py — 共享运行时（CLI/Web 共用状态流转）
...
world/
  └── world_builder.py        — 场景管理（默认场景 + scene.md 动态写回）
ui/
  ├── web_server.py           — Flask 后端（已迁移到共享 Runtime）
...
核心设计：`ScenesoulRuntime` 维护两个独立的消息列表（`brain_messages` 和 `narrator_messages`），各自独立管理 system + 对话历史。`ScenesoulLoop`（CLI）和 `ui/web_server.py`（Web）都复用同一套 Runtime 状态流转。

关键点：
- Agent 接收外部传入的 messages 列表，不内建消息历史
- `BrainContextBuilder` 每次调用时动态重写 system prompt 中的 `【当前状态】` 和 `【世界的场景】` 区块
- 界说输出不含 `[当前状态]` 头部，由 Runtime 通过 `build_state_header()` 拼接后写入大脑消息列表
...
`[ui/web_server.py](ui/web_server.py)` 是 Flask 后端，提供三个 API（`/api/status`、`/api/send`、`/api/think`）。当前已迁移为直接调用 `ScenesoulRuntime`，与 CLI 共用同一套状态流转逻辑。
...
- **驱动力系统**: 从 soul.md 的 traits 字段读取初始值（自定义中文特质名），界说通过 `update_drives` 工具更新，Runtime 统一做数值 clamp（-100~100）
...
- `WorldBuilder` 当前提供 `get_default_scene()` 与 `update_scene()`（写回 profile 的 `scene.md`）
- `settings.json` 作为非 profile 模式的默认回退配置
...
pytest                    # 运行所有测试
pytest -q                 # 简洁模式
pytest tests/test_runtime.py  # 运行单个测试文件
...
- [docs/README.md](docs/README.md) — 文档总览与状态表
- [docs/architecture/system-architecture.md](docs/architecture/system-architecture.md) — 系统架构
- [docs/architecture/runtime-state-flow.md](docs/architecture/runtime-state-flow.md) — Runtime 状态流转
- [docs/guides/configuration.md](docs/guides/configuration.md) — 完整配置模板
